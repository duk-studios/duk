...
#include <duk_renderer/text/text_mesh_builder.h>
...
static void update_mesh(DynamicMesh* mesh, const std::vector<Line>& lines, const FontAtlas* atlas, uint32_t fontSize, const glm::vec2& textBoxSize, float lineHeight, TextVertAlignment vertAlignment, TextHoriAlignment horiAlignment,
                        float pixelsPerUnit) {
    ...
    for (const auto& line: lines) {
        ...
        for (const auto& word: line.words) {
            for (auto chr: word) {
                ...
                mesh->write_vertices(VertexAttributes::POSITION, positions);
                ...
                mesh->write_vertices(VertexAttributes::UV, texCoords);
...
std::shared_ptr<DynamicMesh> allocate_text_mesh(MeshBufferPool* meshBufferPool, uint32_t textCount, bool dynamic) {
    ...
    DynamicMeshCreateInfo dynamicMeshCreateInfo = {};
    dynamicMeshCreateInfo.meshBufferPool = meshBufferPool;
    dynamicMeshCreateInfo.attributes = VertexAttributes({VertexAttributes::POSITION, VertexAttributes::UV});
    ...
    return std::make_shared<DynamicMesh>(dynamicMeshCreateInfo);
}
