...
vec3 duk_calculate_phong_lights(in Lights lights, in float shininess, in vec3 position, in vec3 normal, in vec3 view) {
    vec3 lighting = vec3(0);
    for (int i = 0; i < lights.directionalLightCount; i++) {
        lighting += duk_calculate_phong_directional_light(lights.directionalLights[i], shininess, normal, view);
    }

    for (int i = 0; i < lights.pointLightCount; i++) {
        lighting += duk_calculate_phong_point_light(lights.pointLights[i], shininess, normal, view, position);
    }
    return lighting;
}
